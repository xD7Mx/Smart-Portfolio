...
# خمسٌ لا عشر: المخرَجُ صار كاملَ الصفوف، وعشرُ شركاتٍ تُنتج ثلاثَ مئةِ
# سطرٍ يشقّ نقلُها. وخمسةُ قطاعاتٍ متباعدةٍ تكفي لكشف اختلاف الشكل.
SAMPLE = ["1120", "2010", "4190", "8010", "4330"]

# عناوينُ الأقسام كما تُكتب في الصفحة — تُطابَق على النصّ العربيّ.
_H_PERF = re.compile(r"أداء\s*(السهم|الوحدة)")
_H_FIN = re.compile(r"(المؤشرات\s*المالية|المؤشرات\s*المالي)")
# تسمياتُ المؤشرات كما تُكتب في «أرقام» — تُوسَم بنجمةٍ في المخرَج **للعين
# لا للحكم**: الوسمُ تسهيلُ قراءةٍ، والمسبارُ يطبع كلَّ صفٍّ موسوماً أو لا.
_FIN_LABEL = re.compile(
    r"(مكرر\s*الرب|مضاعف|ربحية\s*السهم|القيمة\s*الدفترية|عائد\s*التوز|"
    r"التوزيعات|العائد\s*على|هامش|النمو|السعر\s*إلى|ص\.?\s*ح\.?\s*الأصول)")
...
async def one(sym: str) -> None:
    ...
    # ══ لا حدَّ للطباعة ══ (قِيس في تشغيل المالك)
    # كان الحدُّ ‎22 صفّاً، فطُبع جدولُ «أداء السهم» كاملاً — وهو الذي
    # قرّرنا ألّا يدخل التقييم — وقُطع ما بعده، وفيه **جدولُ المؤشرات
    # المالية نفسُه**: الغرضُ من المسبار كلِّه. فالحدُّ في مسبارِ اكتشافٍ
    # يحجب ما جاء المسبارُ لأجله.
    for cells in rows:
        mark = "★" if cells and _FIN_LABEL.search(cells[0]) else " "
        print(f"    {mark}· " + " | ".join(c[:34] for c in cells[:7]))
