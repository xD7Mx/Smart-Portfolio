#!/usr/bin/env python3
"""السوقُ الرئيسيُّ وحدَه: ‎273 شركةً · ‎23 قطاعاً · صفرُ «نمو» (D387 · D486).
...
# ‏D486 (بأمر المالك: «لدينا 23 قطاعاً… وصناديقُ تبدأ بـ9 عاملتَها نمو»):
# صناديقُ المؤشرات (94xx) في الرئيسة بقطاعها الرسميّ — فالقطاعات 23 كما
# قال المالك من البداية، والشركاتُ 273 دون الصناديق.
from app.data.universe import is_etf
_cos = [s for s in _mm if not is_etf(s)]
check(len(_cos) == 273, "١ الدليلُ الرسميُّ ‎273 شركة — كما أقرّ المالك",
      f"{len(_cos)} شركة")
check(len(_secs) == 23,
      "١ب و‎23 قطاعاً — منها صناديقُ المؤشرات المتداولة (D486)",
      f"{len(_secs)} قطاعاً")
check(not [s for s in _mm if str(s).startswith("9") and not is_etf(s)],
