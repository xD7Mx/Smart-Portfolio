...
# ── ١٠ · هامادا تطلب دَيناً لا التزامات ─────────────────────────────────
# الاشتقاقُ من نسبة المديونية يعطي **كلَّ** الالتزامات (ذممٌ · مخصّصاتٌ ·
# زكاة)، فترتفع D/E فتنخفض بيتا القطاع زيفاً. والرسميُّ يحمل `total_debt`
# مجموعاً من القروض والإيجارات — فيُقدَّم، ويبقى الاشتقاقُ طبقةً ثانية.
from app.services import lastgood as _lg  # noqa: E402

_c.set("stmt:4030.SR", {"periods": [{"equity": 100.0, "debt_ratio": 40.0,
                                     "total_debt": 10.0}]}, 3600)
lev = sb._leverage_for("4030", _c, _lg)
check(lev == (10.0, 100.0),
      "١٠ الدَّينُ المفصَحُ يتقدّم الاشتقاقَ من نسبة الالتزامات", str(lev))

_c.set("stmt:4031.SR", {"periods": [{"equity": 100.0, "debt_ratio": 40.0}]}, 3600)
lev2 = sb._leverage_for("4031", _c, _lg)
check(lev2 is not None and abs(lev2[0] - 66.6667) < 1e-3,
      "١٠ب وبلا دَينٍ مفصَحٍ يبقى الاشتقاقُ طبقةً ثانيةً — لا فراغ", str(lev2))

_c.set("stmt:4032.SR", {"periods": [{"equity": 100.0}]}, 3600)
check(sb._leverage_for("4032", _c, _lg) is None,
      "١٠ج ومن لا هذا ولا ذاك يُترك — ولا تُفترَض رافعةٌ صفرية")

# وأثرُه في الرقم: D/E الأصغرُ يرفع البيتا المنزوعة — لا يخفضها.
bu_debt = sb.unlever(1.2, 10.0, 100.0, P.tax)
bu_liab = sb.unlever(1.2, 66.6667, 100.0, P.tax)
check(bu_debt > bu_liab,
      "١٠د والفرقُ ليس تجميلاً: التزاماتٌ محسوبةٌ دَيناً تخفض بيتا القطاع",
      f"دَين {bu_debt:.4f} · التزامات {bu_liab:.4f}")
