...
for (const [name, path] of STOPS) {
  ...
  /* ══ خيارا الدورية: سنويٌّ وربعيّ ══ (بأمر المالك · D259)
     أُزيل المُبدِّلُ يومَ كان الربعيُّ يخرج فارغاً من ياهو. وقد صار له
     مصدرٌ رسميّ، فعاد — ويُقاس في متصفّحٍ حقيقيّ: الزرّان موجودان
     بنصّهما، وأحدُهما مضغوطٌ (‏aria-pressed) فيُعرَف الفرعُ المعروض. */
  if (name === "البيانات المالية") {
    const sw = await page.evaluate(() => {
      const bs = [...document.querySelectorAll("button")]
        .filter(b => ["سنوي", "ربع سنوي"].includes((b.textContent || "").trim()));
      return bs.map(b => ({ t: (b.textContent || "").trim(),
                            on: b.getAttribute("aria-pressed") === "true" }));
    });
    say(sw.length === 2, "خيارا الدورية ظاهران — سنويّ وربعيّ",
        sw.map(x => x.t).join(" · "));
    say(sw.filter(x => x.on).length === 1,
        "وواحدٌ منهما مضغوطٌ فيُعرَف الفرعُ المعروض",
        sw.filter(x => x.on).map(x => x.t).join("") || "لا شيء");
  }

  /* ══ شريطُ الخلاصة: سطرُ محلّلٍ واحد ══ (بأمر المالك)
     «ألغِ الدرجة لأنها موجودة ولا أريد تكراراً، وألغِ السنوات لأنها
     فوضوية». فيُقاس الشريطُ نفسُه في متصفّحٍ حقيقيّ: جملةُ الحكم فيه،
     ولا `/100` ولا «سنوات» — والتكرارُ لا يُكتشف بقراءة الشيفرة بل
     بقياس ما يصل الشاشة. ولا أسلوبَ اعتذارٍ أو دعوةٍ لاكتشاف سبب:
     المصادرُ رسميةٌ من مُصدِرها فلا موضعَ لحاشيةٍ تفسيرية. */
  if (name === "البيانات المالية") {
    const bar = await page.evaluate(() => {
      const el = [...document.querySelectorAll("div")]
        .find(d => (d.textContent || "").trim().startsWith("قوائم متينة")
                   && d.children.length <= 2);
      if (!el) return null;
      const t = (el.textContent || "").trim();
      return { text: t, lines: t.split(/\n/).filter(Boolean).length };
    });
    say(!!bar, "شريطُ خلاصة القوائم ظاهرٌ ليُقاس", bar ? bar.text.slice(0, 40) : "");
    if (bar) {
      say(!/\/100|\d+\s*\/\s*100/.test(bar.text),
          "ولا درجةَ فيه — معروضةٌ في بطاقة التقييم فلا تُكرَّر", bar.text.slice(0, 60));
      say(!/سنوات|سنة/.test(bar.text),
          "ولا عدَّ سنواتٍ — ترويسةُ الجدول فوقه تحملها", bar.text.slice(0, 60));
      say(bar.lines === 1 && bar.text.length <= 140,
          "وسطرٌ واحدٌ لا يزيد", `${bar.lines} سطر · ${bar.text.length} حرفاً`);
      say(!/(اكتشف|السبب|راجع|تحقّق|نعتذر|غير متوفّر)/.test(bar.text),
          "وبلا دعوةٍ لاكتشاف سببٍ ولا اعتذارٍ عن مصدر", bar.text.slice(0, 60));
    }
  }
