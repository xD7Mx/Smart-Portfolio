...
_NM_B = _nm("1010")
# ══ والقاعدةُ انقلبت بمبدأٍ أعلى ══ (D370)
# قضى المالك: كلُّ مُصدِرٍ في الرئيسيّ ملزَمٌ بالإفصاح أيّاً كان قطاعُه،
# فلا امتناعَ مفهوم. والبنكُ **ينشر** مصروفَ العمولات الخاصّة، والريتُ
# **ينشر** إضافاتِ العقارات الاستثمارية — فإعفاؤهما كان تغطيةً لعجزِ
# خريطتنا باسم «طبيعةِ القطاع». فيُحرَس المبدأُ لا العُرف: بنكٌ وريتٌ
# **لا يُعفَيان من شيء**، وما لم يُنقَل اسمُه بعدُ يبقى ناقصاً معلَناً.
check(_ar("1010") == "bank" and _NM_B == ()
      and "interest_expense" in _miss(_ROWS9, "1010"),
      "٩ي بنكٌ لا يُعفى من بند — مُصدِرٌ مُلزَمٌ بالإفصاح (D370)",
      f"نمط={_ar('1010')} · إعفاءات={len(_NM_B)}")
check(_ar("9400") == "fund"
...
check(_ar("4330") == "reit" and _nm("4330") == ()
      and "capex" in _miss(_ROWS9, "4330"),
      "٩م وريتٌ لا يُعفى من الرأسماليّ — ينشره «إضافاتِ عقاراتٍ استثمارية»",
      f"إعفاءات={len(_nm('4330'))}")
# ولا يبقى معفىً إلا ما لا يُصدِر قوائمَ أصلاً — فيُحرَس أن الإعفاءَ
# **مقصورٌ** على صنفٍ واحدٍ ولا يتسلّل إلى صنفٍ مُصدِر.
_EXEMPT = [a for a in ("bank", "reit", "insurance", "financial", "commodity",
                       "utility", "telecom", "retail")
           if _nm({"bank": "1010", "reit": "4330", "insurance": "8010",
                   "financial": "1111", "commodity": "2222",
                   "utility": "5110", "telecom": "7010",
                   "retail": "4001"}.get(a, "2222"))]
check(_EXEMPT == [],
...
# منشورةً: السعر ÷ مضاعفِ الدفترية من لقطة «تداول». ولا يُستبدَل منشورٌ
# بمشتقّ، ويُعلَن أصلُ كلّ مشتقٍّ في `derived_inputs`.
_NOBV = [{"as_of": f"{y}-12-31", "year": y, "shares_outstanding": 1e8,
          "revenue": 1.5e9, "net_income": 9e7, "eps": 0.9,
          "total_assets": 2e9, "operating_cash_flow": 1.4e8, "capex": 4e7,
          "ending_cash": 2e8, "pretax_income": 1e8, "total_debt": 3e8}
         for y in (2023, 2024, 2025)]
_RD = _fv({"pe_ratio": 12.0, "price_to_book": 1.6, "beta": 0.9, "eps": 0.9},
          11.0, sector_avg_pe=14.0, sector_avg_pb=1.8, periods=_NOBV,
          archetype="consumer_cyclical", symbol="4006", peer_count=8)
check(_RD.get("value") is not None,
      "٩ه٢ لا امتناعَ لغياب دفتريةٍ نملك مضاعفَها منشوراً",
      f"قيمة={_RD.get('value')}")
check(any("مضاعفِ الدفترية المنشور" in d
          for d in (_RD.get("derived_inputs") or [])),
      "٩ه٣ ويُعلَن أصلُ المشتقّ بالحرف — لا رقمٌ مجهولُ الأصل",
      str((_RD.get("derived_inputs") or ["—"])[0])[:56])
check(not any("دائم" in str(m.get("name")) for m in _RD.get("methods") or []),
      "٩ه٤ ولا يُختلَق عائدُ حقوقٍ بلا حقوق — المسارُ يسقط ولا يُلفَّق")

# ── ٩ص٢ · السحبُ بالبنية لا بالصنف (D399) ──────────────────────────────
# صنَّفت «تداول» رسن (‏8313) تأميناً وهي منصّةُ تقنيةٍ تنشر إيراداً
# عادياً بعمودٍ واحد. وقاعدةُ سحبِ إيراد المؤمِّن تسحب على الصنف، فكانت
# ستُتلف إيراداً سليماً. فالسحبُ يُشترط بوجود **قائمةِ مؤمِّنٍ بشطرَين**
# في الملفّ نفسِه — بنيةٌ تُقاس لا صنفٌ يُفترَض.
try:
    ...
    _n3, _kept = -1, None
check(_n3 == 0 and _kept == 250000.0,
      "٩ص٢ منصّةُ تقنيةٍ مصنَّفةٌ تأميناً تبقي إيرادَها — السحبُ بالبنية",
      f"سُحب={_n3} · إيراد={_kept}")
