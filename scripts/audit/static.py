...
def check_flat_page() -> None:
    ...
    # والعطبُ نفسُه تكرّر ثالثةً حين فُصل «هدف المحللين» عن «السعر العادل»
    # (‏D176): سقط الحارسُ على تصحيحٍ لا على عطب. فالثابتُ المحروس صار
    # **بنيويّاً لا لفظياً**: بطاقتان تتصدّران، إحداهما تحمل الدرجة
    # (‏`fin.score`) والأخرى الرقمَ المعروض (‏`data.fair_value`) — أيّاً
    # كانت تسميتُهما في الشاشة.
    if ("data.fair_value" not in body
            or not re.search(r"fin\.score|financial\?\.score", body)):
...
def check_symbol_consistency() -> None:
    ...
    if a.exists():
        ...
        # ══ الشرطُ: لا نسخةَ محلّية — لا وجوبُ العرض ══ (D208)
        # كان يشترط ظهورَ المقياسين معاً في هذه الشاشة، فحين أمر المالكُ
        # بحذف شريط هدف المحلّلين منها سقط الفحصُ على تنفيذٍ صحيح. والذي
        # يحرسه في الحقيقة أن الشاشةَ **لا ترسم مقياساً بيدها** — فإن عرضت
        # مقياساً فمن التعريف المشترك، وإن لم تعرض فلا شيء.
        if "linear-gradient(90deg, var(--gauge-neg)" in t:
            note("S-SYMBOL", "frontend/src/components/analysis/AnalysisPanel.tsx",
                 "الشاشةُ ترسم مقياسَ انحرافٍ بيدها — نسخةٌ ثانية تتباعد "
                 "عن تعريفها المشترك مع أوّل تعديل")
