...
# ── ٩ · «مرتفعة» قابلةٌ للبلوغ ───────────────────────────────────────
# على السوق الحقيقيّ خرجت ‎247 شركةً بلا واحدةٍ مرتفعةِ الثقة — لأنّ مقياس
# التشتّت كان مدىً كاملاً يحكمه شاذٌّ واحد. ودرجةٌ لا تُبلَغ أبداً ليست
# درجةً بل عيبٌ صامت. فيُشترط بلوغُها في قطاعٍ متقاربٍ عشرِ نظائرَ.
peers10 = [10.0, 10.4, 10.8, 11.0, 11.2, 11.5, 11.8, 12.0, 12.4, 12.8]
pbs10 = [round(p / 10, 3) for p in peers10]
dense = SectorTable(rows("كثيف", peers10, pbs10))
d = relative_value(sector="كثيف", price=110, pe=11.0, pb=1.10,
                   book_value=100, table=dense)
check(d["confidence"] == "مرتفعة",
      "٩ قطاعٌ متقاربٌ بعشرِ نظائرَ يبلغ «مرتفعة»",
      f"الثقة {d['confidence']} · مسارات {len(d['paths'])}")
