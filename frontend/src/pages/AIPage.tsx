                  ...
                  <tr key={c.symbol} className="border-b border-[var(--hairline)]/40 hover:bg-[var(--field)]">
                    ...
                    <td className="td tabular-nums"
                      title={c.fair_value == null && c.rel_value != null
                        ? `التقييم النسبي ${c.rel_low}–${c.rel_high} · ثقة ${c.rel_conf}`
                        : ""}
            ...
            {companies.map((c: any) => (
              <div key={c.symbol} className="rounded-xl border border-[var(--hairline)] panel p-3 space-y-2.5">
                ...
                <div className="grid grid-cols-3 gap-1 text-center">
                  ...
                  <div><div className="text-[9.5px] text-[var(--ink-muted)] mb-0.5">{c.fair_value == null && c.rel_value != null ? "التقييم النسبي" : "هدف المحللين"}</div><div className="font-semibold tabular-nums text-xs" style={{ color: c.fair_value == null && c.rel_value != null ? "var(--ink-muted)" : "var(--ink)" }} title={c.fair_value == null && c.rel_value != null ? `التقييم النسبي ${c.rel_low}–${c.rel_high} · ثقة ${c.rel_conf}` : ""}>{c.fair_value != null ? c.fair_value.toFixed(2) : c.rel_value != null ? c.rel_value.toFixed(2) : "—"}</div></div>
