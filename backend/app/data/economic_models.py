...
MODEL_OF_SECTOR: dict[str, str] = {
    ...
    # اسمٌ في خريطة المادة ٢ لا مقابلَ له في قاعدة السوق اليوم؛ يُربَط
    # سلفاً فلا تيتم شركةٌ إن ظهر غداً. ومؤشّراتُه في الخريطة نفسُها
    # مؤشّراتُ النموذج التشغيليّ.
    "الخدمات الاستهلاكية الدورية": OPERATING,
    "النقل": OPERATING,
...
COMPONENTS: dict[str, dict[str, tuple]] = {
    ...
    CYCLICAL: {
        # المادة ٢ تجعل العائدَ على رأس المال المستثمر **المحورَ الأكبر**
        # في الطاقة والمواد الأساسية، والعائدَ على حقوق الملكية مسانداً.
        # فأُدخل `roic` بوزن المحور الأوّل كما في النموذج التشغيليّ،
        # وقُيست الخمسةُ الباقية بـ0.75 فحفظت نسبتَها بعضِها إلى بعض
        # حرفاً بحرف — لا معايرةَ لرفع درجة، بل مطابقةُ خريطة القطاعات.
        "quality": (
            ("roic", "العائد على رأس المال المستثمر", 0.25, "higher"),
            ("roe", "العائد على حقوق الملكية", 0.19, "higher"),
            ("net_debt_ebitda", "الدَّين إلى الأرباح التشغيلية", 0.15, "lower"),
            ("interest_coverage", "تغطيةُ الفوائد", 0.15, "higher"),
            ("earnings_stability", "استقرارُ الربح عبر الدورة", 0.15, "lower"),
            ("operating_margin", "الهامشُ التشغيليّ", 0.11, "higher"),
        ),
        "dividend": _DIVIDEND,
        "growth": (
    ...
    REAL_ESTATE: {
        "quality": (
            # المادة ٢: العائدُ على حقوق الملكية أساسيّ والرافعةُ مساندة.
            # وكانا متعادلَين فلم يكن للأساسيّ صدارة؛ نُقلت خمسُ نقاطٍ
            # من المساند إلى الأساسيّ ولم يُمسّ سواهما.
...
ALLOWED_REPEAT: dict[tuple[str, str], str] = {
    ...
    # المادة ٢ تسمّي للطاقة والمواد الأساسية: «ROIC أساسي؛ ROE مساند» —
    # وهو نصُّ ما أُجيز للنموذج التشغيليّ. فالإذنُ يمتدّ بالنصّ نفسه ولا
    # يُخترع: `roic` على رأس المال المستثمَر كلِّه و`roe` على حقوق
    # الملكية وحدها، فيفترقان حين تثقُل الرافعة.
    ("CYCLICAL", "quality"): "PROFITABILITY_ON_CAPITAL",
}
