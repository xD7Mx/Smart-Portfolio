"""
قائمة مصادر الأخبار الرسمية المعتمدة — the injected, canonical registry of
reputable Arabic/financial news outlets the site trusts, with each outlet's
REAL domain (used by the frontend to render the outlet's logo, resolved from
the domain itself via the same Clearbit/favicon convention already used for
company logos — never an invented image URL).

This is the site's news-source policy file ("نهزم العشوائية"): admission
ranking (TRUSTED_SOURCES in news_fetcher) derives from THIS list, and the
frontend's newsSources.ts mirrors it — one canonical registry, two layers.

Aliases cover how Google News RSS actually labels each outlet (Arabic and
English variants seen in real feed responses).
"""
...
# alias (exact string as it appears in feeds) -> canonical entry
_ALIAS_INDEX: dict[str, dict] = {
    alias: entry for entry in NEWS_SOURCES for alias in entry["aliases"]
}


def match_source(source: str | None) -> dict | None:
    """Return the canonical outlet entry for a feed's source label, or None
    if the outlet isn't in the approved registry."""
    if not source:
        return None
    s = source.strip()
    if s in _ALIAS_INDEX:
        return _ALIAS_INDEX[s]
    # tolerate labels like "أرقام - Argaam" or extra whitespace
...
TRUSTED_SOURCE_ALIASES: set[str] = set(_ALIAS_INDEX.keys())
