...
def _persist_valuation(symbol: str, data: dict) -> None:
    ...
    # ══ ما يُشتقّ من السعر يحتاج مقياسَه لا نتيجتَه ══ (D239)
    # المكرّرُ ومضاعفُ الدفترية **دالّتا سعر**: يتغيّران كلَّ يومٍ وإن لم
    # تتغيّر الشركة. فتخزينُ نتيجتِهما يعني أن الفرزَ يعرض مكرّرَ أمسِ
    # بسعر اليوم. فيُخزَّن معهما **مقياسا الشركة** — ربحيةُ السهم والقيمةُ
    # الدفترية — ليُعاد الحسابُ عند التقديم بالسعر الحاضر بلا نداء.
    # وحدّا العام يُخزَّنان لأنهما مصدرُ صفحة السهم نفسُه، فلا يُقاسان
    # مرّةً من تاريخٍ ومرّةً من المزوّد.
...
class MarketDataService:
    ...
    async def get_financials(self, symbol: str, allow_supplement: bool = True) -> Optional[dict]:
        ...
        if rows:
            ...
            # ══ وأحدثُ ربعٍ يُسلَّم معها ولا يُدمَج فيها ══ (D412)
            # قِيس على الخادم: **25 من 30** ورقةً تملك ربعيّاً أحدثَ ممّا
            # تُقيَّم به — وسيطُ المستعمَل ‎266 يوماً ووسيطُ المتاح ‎85.
            # فكنّا نقيّم بأرقامٍ عمرُها ثلاثةُ أضعافِ ما نملك، ونَصِمُها
            # بالشيخوخة بحقٍّ، والعلّةُ عندنا لا عند المصدر.
            # ولا يُدمَج الربعيُّ في السلسلة السنوية: مستهلكوها يقيسون
            # النموَّ المركّبَ على عددها ويقرؤون `periods[-1]` سنةً كاملة
    ...
    async def _complete(self, symbol: str, rows: list, allow_supplement: bool,
        ...
        if with_yahoo:
            ...
            # ومسطرةُ الصنف تُقرأ هنا كذلك: بندٌ لا معنى له في نموذج
            # الورقة ليس نقصاً يُطلَب من مزوّد (‏D349) — فلا حصّةَ تُحرَق
            # لتكلفةِ تمويلِ بنكٍ لا ينشرها، ولا لقوائمِ صندوقٍ مؤشِّر.
            if SM.missing(rows, symbol):
                try:
