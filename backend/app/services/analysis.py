...
async def analyze_company(symbol: str, name: str | None = None, db=None, allow_supplement: bool = True) -> Optional[dict]:
    ...
    # ══ الإسنادُ يعود بشرطين، لا بلا شرط ══ (D175)
    # جُعل تقديرُ التطبيق سنداً حيث يغيب هدفُ المحلّلين، فقِيس على ‎١٢٠
    # شركةً لها هدفٌ نقيس عليه: وسيطُ الانحراف ‎٤٤٪ وضمن ‎±٢٥٪ ‎٢٦٪ فقط.
    # والأخطرُ شكلُ الخطأ: سبعٌ من أسوأ عشرٍ تنهار إلى الصفر — ‎0.07 ريالٍ
    # لورقةٍ هدفُها ‎35.
    #
    # وعطبان اجتمعا:
    #   · المحرّكُ بلا أرضيةِ أصولٍ تقابل سقفَ الدفترية، فينهار متى
    #     انضغطت الأرباح. عولج في `fair_value` بأرضيةِ نصفِ الدفترية.
    #   · و**المحرّكُ كان يَسِم شذوذَه بنفسه** (`implausible` تشتعل دون
    #     ‎0.40× السعر) وأخذتُ قيمتَه متجاهلاً وسمَه. فالخطأُ الثاني خطئي
    #     أنا: مصدرٌ يعلن تحفّظَه فيُقرأ رقمُه ويُترك تحفّظُه.
    #
    # فالإسنادُ لا يُقبل إلّا من تقديرٍ **لا يشكّ في نفسه**: غيرِ شاذّ،
    # ولا من مسارٍ واحدٍ بلا شاهد، ولا منخفضِ الثقة.
    _fv_own = None
    if isinstance(_fv, dict):
        _v = _fv.get("value")
        _trusted = not (_fv.get("implausible") or _fv.get("single_path")
                        or _fv.get("confidence") == "منخفضة")
        if isinstance(_v, (int, float)) and _v > 0 and _trusted:
            _fv_own = _v
    ...
    result = {
        ...
        "governance_provenance": {
            ...
            "مصدر الأساسيات": "ياهو" + ("‏ + سهمك" if allow_supplement else ""),
