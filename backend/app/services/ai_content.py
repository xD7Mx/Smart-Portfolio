...
async def stock_opinion(symbol: str, name: str, analysis: dict, headlines: list[str] | None = None) -> dict | None:
    ...
    per symbol."""
    key = f"ai:opinion:{date.today().isoformat()}:{symbol}"
    ...
    lines = [
        ...
        # القيمةُ العادلة من محرّكنا — محسوبةً من قوائم الشركة نفسها، وهي
        # الرقمُ الذي تعرضه صفحة الشركة. وكان يُمرَّر هنا `target_mean_price`
        # باسم «القيمة العادلة»، فيكتب النموذجُ في تحليله رقماً يخالف
        # البطاقةَ التي أمامه. رقمان باسمٍ واحد على شاشتين. (D147)
        f"القيمة العادلة (محسوبة من قوائم الشركة): {analysis.get('fair_value')}"
        ...
        # هدفُ المحلّلين يبقى معروضاً — لكن باسمه وبوصفه الصحيح، سؤالاً
        # آخر لا بديلاً عن الأوّل.
        f"متوسط أهداف المحللين (تنبّؤ سعرٍ لسنة، لا قيمة جوهرية): "
        f"{f.get('target_mean_price')}" if f.get("target_mean_price") else "",
...
هل أنت متفائل أم متشائم بشأن {name}:{symbol} حاليًا؟ حلل أساسياته (مكرر الربحية P/E، نمو ربحية السهم EPS) باستخدام المقاييس الرئيسية أدناه، وقارن السعر بالقيمة العادلة المحسوبة أعلاه، ولاحظ أي إشارات فنية مهمة من المؤشرات الفنية. شاركنا رأيك العام في السوق تجاه هذا السهم واشرح أسبابك.
...
{{
  "sentiment_label": "متفائل بحذر / متفائل / متشائم / محايد (اختر الأقرب فقط)",
