...
async def stock_opinion(symbol: str, name: str, analysis: dict, headlines: list[str] | None = None) -> dict | None:
    ...
    lines = [
        ...
        f"القيمة العادلة (متوسط أهداف المحللين): {f.get('target_mean_price')}" if f.get("target_mean_price") else "",
...
هل أنت متفائل أم متشائم بشأن {name}:{symbol} حاليًا؟ حلل أساسياته (مكرر الربحية P/E، نمو ربحية السهم EPS) باستخدام المقاييس الرئيسية أدناه، وراجع إجماع السعر العادل المستهدف، ولاحظ أي إشارات فنية مهمة من المؤشرات الفنية. شاركنا رأيك العام في السوق تجاه هذا السهم واشرح أسبابك.
