...
def _ebitda_of(p: dict) -> float | None:
    """الأرباحُ التشغيلية قبل الإهلاك — من بندٍ وارد أو لا شيء.

    ══ الربحُ التشغيليّ ليس EBITDA ولو تحفّظاً ══ (بأمر المواصفة · المادة ٣)
    كان يُستعمل الربحُ التشغيليّ وحده حين يغيب الإهلاك، بحجّة أنه «أشدُّ
    تحفّظاً»: مقامٌ أصغر يرفع نسبةَ الدَّين فتبدو الشركةُ أثقل. وذلك
    خلطُ بابين: التحفّظُ يخفض **تقديراً**، وهذا يُسمّي رقماً باسم رقمٍ
    آخر — فيُنشر «صافي الدين إلى الأرباح التشغيلية ‎7.2×» وهو محسوبٌ على
    مقامٍ ليس هو. والقارئُ لا يرى الفرق.

    فالاشتقاقُ صار قاطعاً: بندُ `ebitda` كما ورد، أو الربحُ التشغيليّ
    **مع** إهلاكٍ واصل. وحين يغيب الإهلاك يُعاد `None` ويُعلَن المؤشّر
    غيرَ متاح — لا لأن الشركة سليمة، بل لأن الرقمَ لم يصل.
    """
    v = _n(p, "ebitda")
    if v is not None:
        return v
    op, dep = _n(p, "operating_income"), _n(p, "depreciation")
