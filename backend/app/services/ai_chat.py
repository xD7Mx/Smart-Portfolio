...
# كلماتٌ عامّةٌ ترد في أسماء كثيرة — مطابقتُها وحدَها تُحيل السؤال إلى شركةٍ
# لم تُذكر. هي نفسُها قائمةُ `resolve_company` كي لا يفترق المُطابِقان.
_STOP_WORDS = {"شركه", "مصرف", "بنك", "مجموعه", "السعوديه", "القابضه",
               "العربيه", "للتسويق", "للاستثمار", "الوطنيه", "التعاونيه",
               "الخليج", "اسواق", "الصناعات", "الكهربائيه"}


def _mentions(question: str, name: str | None, symbol: str | None) -> bool:
    """أذُكرت هذه الشركةُ في السؤال؟ — بالتطبيع لا بالحرف.

    الرمزُ يُطابَق كما هو، والاسمُ كاملاً أوّلاً، ثمّ كلمةٌ مميّزةٌ منه
    (أربعةُ أحرفٍ فأكثر وليست من الكلمات العامّة).
    """
    from app.services.ai_chat_rules import _n
    q = _n(question or "")
    if not q:
        return False
    if symbol and str(symbol) in (question or ""):
        return True
    key = _n(str(name or ""))
    if len(key) < 3:
        return False
    if key in q:
        return True
    return any(len(tok) >= 4 and tok not in _STOP_WORDS and tok in q
...
def _market_context(question: str) -> dict:
    ...
    try:
        ...
        if rows:
            ...
            # ══ «العثيم» ليست «أسواق عبدالله العثيم» ══ (D222)
            # كانت المطابقةُ نصّيةً خاماً تشترط ورودَ الاسم **كاملاً** في
            # السؤال. فسأل المالكُ عن «العثيم» فلم يُطابَق شيء، فسقط الاختيارُ
            # إلى «أبرز العوائد»، فلم تصل الشركةُ إلى صقر — فنفى وجودَها في
            # السوق كلِّه. ونفيُه كان صادقاً عن سياقٍ أعميناه نحن.
            # والمطابقةُ الآن مُطبَّعةٌ على كلمةٍ مميّزة، كما في `resolve_company`.
            picked = [r for r in rows if _mentions(question, r.get("name"), r.get("symbol"))]
            if not picked:
            ...
            out["بيانات الفرز"] = [{
                ...
                # ══ صقرُ يرى خلاصةَ المحرّكَين بأسمائها في التطبيق ══ (D437)
                # قال المالك: «صقرُ امتدادٌ للتطبيق بكامل بياناته». وكان هذا
                # الصفُّ يحمل السعرَ وRSI والعائد — ولا يحمل السعرَ العادل ولا
                # فجوتَه ولا ثقتَه ولا القرار، وهي ما يقوم عليه التطبيق.
                # فيُسأل عن السعر العادل ولا رقمَ من المحرّك في يده. وكانت
                # الدرجةُ «درجة الحوكمة» والتطبيقُ يسمّيها «درجة الجودة المالية».
                "درجة الجودة المالية": r.get("finance_score"),
                "السعر العادل": r.get("fair_value"),
                "الفجوة عن السعر العادل٪": r.get("fair_value_upside_pct"),
                "ثقة السعر العادل": r.get("fair_value_conf"),
                "هدف المحللين": r.get("analyst_target"),
                "القرار": r.get("decision"),
...
async def build_context(db, question: str) -> dict:
    ...
    try:
        ...
        pass
    # ══ الشركةُ تُحسم قبل السؤال ══ (D222)
    # قال صقرٌ عن سهمٍ **في محفظة المالك**: «لا يوجد ضمن مراكزك». والمراكزُ
    # تصل كاملةً — لكنّ النموذجَ يمسح سياقاً طويلاً فينفي ما لم يره. فلا
    # يُترك التعرّفُ لمسحه: تُحسم الشركةُ بمطابقةٍ حتمية، ويُوضع صفُّها في
    # مفتاحٍ مستقلٍّ لا يُخطئه، ويُلزَم بألّا ينفيَ وجودها.
    try:
        from app.services.ai_analyst import resolve_company
        _hit = resolve_company(question or "", ctx)
        if _hit and (_hit.get("name") or _hit.get("symbol")):
            ctx["الشركة المقصودة في السؤال"] = {
                "الاسم": _hit.get("name"),
                "الرمز": _hit.get("symbol"),
                "من": _hit.get("source"),
                "بياناتها": _hit.get("row") or {},
                "ملاحظة": "حُسمت بمطابقةٍ حتمية داخل التطبيق — موجودةٌ في "
                          "بياناتك، فلا يصحّ نفيُ وجودها.",
            }
    except Exception as e:                                        # noqa: BLE001
        logger.warning(f"chat: تعذّر حسمُ الشركة — {e}")
    return ctx
...
2ب. **ونفيُ الوجود ليس امتناعاً بل دعوى.** إن ورد في السياق مفتاحُ «الشركة المقصودة في السؤال» فالشركةُ **موجودةٌ في بيانات المالك يقيناً** — حُسمت بمطابقةٍ حتمية لا بمسحك — فيحرم أن تقول إنها ليست في محفظته أو في السوق. وإن نقصك عنها **حقلٌ بعينه** فسمِّ الحقلَ الناقصَ وحدَه وأجب بما توفّر؛ ولا تُبطل السؤال كلَّه بحجّة نقصٍ جزئيّ. وقبل أن تنفيَ وجودَ شركةٍ، فتّش قائمةَ «المراكز» بالاسم الجزئيّ (‏«العثيم» تعني «أسواق عبدالله العثيم»).
